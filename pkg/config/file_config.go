...
var (
	runtimeConfigPathDefault = "/run/eke/eke.yaml"
)

// InitRuntimeConfig generates the runtime /run/eke/eke.yaml
func (rules *ClientConfigLoadingRules) InitRuntimeConfig(ekeVars constant.CfgVars) error {
	rules.EkeVars = ekeVars
...
func (rules *ClientConfigLoadingRules) ParseRuntimeConfig() (*v1beta1.ClusterConfig, error) {
	...
	if rules.EkeVars.DefaultStorageType == "kine" {
		storage = &v1beta1.StorageSpec{
			Type: v1beta1.KineStorageType,
			Kine: v1beta1.DefaultKineConfig(rules.EkeVars.DataDir),
		}
	}
	if rules.RuntimeConfigPath == "" {
		rules.RuntimeConfigPath = runtimeConfigPathDefault
	...
	case "":
		...
		f, err := os.Open(constant.EkeConfigPathDefault)
		if err != nil {
	...
	if cfg.Spec.Storage.Type == v1beta1.KineStorageType && cfg.Spec.Storage.Kine == nil {
		...
		cfg.Spec.Storage.Kine = v1beta1.DefaultKineConfig(rules.EkeVars.DataDir)
	}
...
func (rules *ClientConfigLoadingRules) writeConfig(yamlData []byte, storageSpec *v1beta1.StorageSpec) error {
	...
	if err != nil {
		return fmt.Errorf("failed to write runtime config to %s (%v): %v", rules.EkeVars.RunDir, rules.RuntimeConfigPath, err)
	}
	return nil
}
